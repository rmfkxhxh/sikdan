...
app.get('/', (req, res) => {
    ...
    if (req.session.valid) {
        res.render('index', {breadcrumbList: ["HOME"], 
        products: products, page: 'event.pug', 
...
app.get('/detail', (req, res) => {
    if (req.session.valid) {
        ...
        res.render('detail', {
            breadcrumbList: ["HOME", '상품상세'], sessionValid: req.session.valid})
    }
})
app.get('/story', (req, res) => {
    if (req.session.valid) {
        res.render('story', {
            breadcrumbList: ["HOME", '스토리'], 
            sessionValid: req.session.valid, 
            userId: req.session.user.Id,
            user : req.session.user
        })
